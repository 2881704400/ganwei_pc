<template>
	<div class="equip-linkage">
		<gw-tabs :navList="tabs" @tabsNavClick="tabClick">
			<div v-for="(tab, index) of tabs" :slot="tab.name">
				<table>
					<thead>
						<tr>
		...
		<Modal v-model="showAdd" title="Common Modal dialog box title">
			...
			<Form :label-width="160">
				<FormItem label="选择联动设备和触发的类型">
					<Cascader :data="listAdd" :load-data="loadListAdd" v-model="selectedAdd" @on-visible-change="getEquips"></Cascader>
				</FormItem>
			</Form>
	...
	export default {
		...
		methods: {
			...
			loadListAdd(item, callback) {
				item.loading = true
				this.Axios.all([this.Axios.post('/api/real/get_ycp', {
						equip_nos: item.m_iEquipNo
					}), this.Axios.post('/api/real/get_yxp', {
						equip_nos: item.m_iEquipNo
					})], {
						equipNo: item.value
					})
