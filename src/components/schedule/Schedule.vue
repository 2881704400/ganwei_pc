				...
				<TabPane label="人员" name="User" class="publicSchedule">
					...
					<Modal v-model="user_modal" title="人员设置" class="ModalUser" :mask-closable="false">
						<Row :gutter="16">
							...
							<!--<Form ref="formValidate" :rules="ruleValidate" style="margin: 65px 0 -10px;" :label-width="100">
								<FormItem label="电话号码:" prop="user_telphone">
									<Input placeholder="请输入电话" v-model="user_telphone" id="phoneAdmin" style="width: 100%;margin-left: -8%;padding-right: 12px;border-radius: 0;border-color: #eee;"></Input>
								</FormItem>
								<FormItem label="短信号码:" prop="user_molphone">
									<Input placeholder="请输入短信" v-model="user_molphone" id="msphoneAdmin" style="width: 100%;margin-left: -8%;padding-right: 12px;border-radius: 0;border-color: #eee;"></Input>
								</FormItem>
								<FormItem label="电子邮箱:" prop="user_email">
									<Input placeholder="请输入电子邮箱" v-model="user_email" id="eamilAdmin" style="width: 100%;margin-left: -8%;padding-right: 12px;border-radius: 0;border-color: #eee;"></Input>
								</FormItem>
							</Form>-->
							...
							<Col span="17"><input type="text" placeholder="请输入电话号码：0000-0000000" v-model="user_telphone" v-on:input="onValidate('phone')" id="phoneAdmin" /></Col>

							<Col span="7">
							<span>短信号码:</span>
							</Col>
							<Col span="17"><input type="text" placeholder="请输入11位短信号码" v-model="user_molphone" v-on:input="onValidate('msphone')" id="msphoneAdmin" /></Col>
							...
							<Col span="17">
							<Select v-model="user_level" placement="bottom" transfer placeholder="无" id="levelAdmin">
				...
				<TabPane label="设备分组范围" name="Equipment" class="publicSchedule">
					<div class="EquipmentLeft">
						...
						<ul class="leftContent">
							<li v-for="(equipUsertItem,equipUserIndex) in equipUser" :key="equipUserIndex" @click.stop="EquipGroupSel(equipUsertItem)" :class="{selectEquipName: equipUsertItem.group_no == selectEquip}">
								...
								<Row v-show="equipUsertItem.isShow">
									...
									<i class="iconfont icon-scheduleSAVE" title="保存" @click.stop="saveEquipGroupName(equipUsertItem,equipUserIndex)"></i>
									</Col>
								</Row>
							</li>
						</ul>
				...
				<TabPane label="管理范围" name="Administration" class="publicSchedule">
					...
					<Modal v-model="AlmReport_modal" title="管理范围" class="ModalUser" :mask-closable="false">
						<Row :gutter="16">
							...
							<Col span="17">
							<Select v-model="AlmReport_Administrator" class="userParentDev" transfer @on-change="onValidateAlmReport()">
							...
							<Col span="17">
							<Select v-model="AlmReport_group_no" class="userParentDev" transfer @on-change="onValidateAlmReport()">
				...
				<TabPane label="周排表" name="Tablerow" class="publicSchedule">
					...
					<Modal v-model="Week_modal" title="周排表" class="ModalWeek" :mask-closable="false">
						<Row :gutter="16">
							...
							<Col span="17">
							<Select v-model="Week_admin" transfer class="userParentDev">
								<Option v-for="item_child in Alarm_user" :value="item_child.Administrator" :key="item_child.Administrator">{{item_child.Administrator}}</Option>
							</Select>
							</Col>
							<Col span="7">
							...
							<Col span="17">
							<Select v-model="Week_week" transfer class="userParentDev">
							...
							<TimePicker type="time" format="HH:mm" :editable="false" v-model="Week_stime" transfer style="outline: none;width: 100%;" @on-change="isTime()"></TimePicker>
							<span id="start_timeIcon">*</span>
							<span id="start_timeFont">开始时间异常</span>
							</Col>
							<Col span="7">
							<span>结束时间:</span>
							</Col>
							<Col span="17">
							<TimePicker type="time" format="HH:mm" :editable="false" v-model="Week_etime" transfer style="outline: none;width: 100%;" @on-change="isTime()"></TimePicker>
							<!--<TimePicker format="HH:mm" placeholder="Select time" style="width: 112px"></TimePicker>-->
							<span id="end_timeIcon">*</span>
							<span id="end_timeFont">结束时间异常</span>
							<span id="end_timeSize" style="margin-top: -20px;right: -70px;;line-height: 20px;">结束时间数<br />值过小</span>
							</Col>
						</Row>
						<div slot="footer">
				...
				<TabPane label="特定日期排表" name="Datetablerow" class="publicSchedule">
					...
					<Modal v-model="Spe_modal" title="特定日期排表" class="ModalWeek" :mask-closable="false">
						<Row>
							...
							<Col span="19">
							<Select v-model="Spe_admin" transfer class="userParentDev" @on-change="onValidateSpeAlmReport()">
								<Option v-for="item_child in Alarm_user" :value="item_child.Administrator" :key="item_child.Administrator">{{item_child.Administrator}}</Option>
							</Select>
							</Col>
							<Col span="5">
							...
							<Col span="19">
							<DatePicker type="datetime" transfer placeholder="开始时间" :editable="false" format="yyyy/MM/dd HH:mm:ss" style="width: 100%;height: 100%;" v-model="Spe_begin_time" class="userParentDev" @on-change="onValidateSpeAlmReport()" @on-clear="onValidateSpeAlmReport_cancel()"></DatePicker>
							</Col>
							<Col span="5">
							<span>结束时间:</span>
							</Col>
							<Col span="19">
							<DatePicker type="datetime" transfer placeholder="结束时间" :editable="false" format="yyyy/MM/dd HH:mm:ss" style="width: 100%;height: 100%;" v-model="Spe_end_time" @on-change="onValidateSpeAlmReport()" @on-clear="onValidateSpeAlmReport_cancel()"></DatePicker>
							<span v-show="!Spe_isDate" id="end_DateIcon">*</span>
							<span v-show="Spe_isDate" id="end_DateSize" style="margin-top: -20px;right: -70px;line-height: 20px;">结束时间数<br />值过小</span>
							</Col>
						</Row>
						<div slot="footer">
	...
	export default {
		...
		data() {
			return {
				...
				user_level: 0,
				user_saveCell: "",
				user_adminFlag: false,
				user_telphoneFlag: true,
				user_molphoneFlag: true,
				user_emailFlag: true,
				ruleValidate: {
					user_telphone: [{
						type: 'number',
		...
		methods: {
			...
			removeAdministrator: function(dt) {
				...
				var WeekAlmReport = this.Alarm_user,
					dtThis = this,
					AlmReportDataLen = this.AlmReportData.length,
					deleteJson = {
				...
				for(var i = 0; i < AlmReportDataLen; i++) {
					if(dtThis.AlmReportData[i].Administrator == dt.Administrator) {
						this.$Modal.warning({
							title: "提示",
							content: '<p style="position: relative;top: -4px;">请先删除管理范围对应项!</p>'
						});
						return false;
					}
				}
				...
				function _success_admin_del() {
					dtThis.filtersArray = dtThis.filtersArray.filter(t => t.Administrator != dt.Administrator);
					msg.success("操作成功");
			...
			saveUpdateAdministrator: function() {
				...
				if(this.user_admin.length > 32) {
					this.$Message.warning("人员名称不能超32个字符");
					return false;
				}
			...
			onValidate(old) {
				switch(old) {
					case "user":
						var dtThis = this,
							dt = document.getElementById("userAdmin").parentNode;
						if(dtThis.user_admin != "" && dtThis.Alarm_user.length != 0 && dtThis.user_admin.trim() != '') {
							for(var i = 0; i < dtThis.Alarm_user.length; i++) {
								if(dtThis.Alarm_user[i].Administrator == dtThis.user_admin) {
									dt.id = "msgInfo";
									this.user_adminFlag = false;
									document.getElementById("user_ok").disabled = "disabled";
									break;
								} else {
									dt.id = "";
									this.user_adminFlag = true;
									if(this.user_adminFlag && this.user_telphoneFlag && this.user_molphoneFlag && this.user_emailFlag) {
										document.getElementById("user_ok").disabled = "";
									}
								}
							}
						} else if(dtThis.user_admin != "" && dtThis.Alarm_user.length == 0 && dtThis.user_admin.trim() != '') {
							dt.id = "";
							this.user_adminFlag = true;
							if(this.user_adminFlag && this.user_telphoneFlag && this.user_molphoneFlag && this.user_emailFlag) {
								document.getElementById("user_ok").disabled = "";
					...
					case "msphone":
						...
						if(value != "") {
							if(str.test(value) || (value.length == 11 && str1.test(value))) {
								document.getElementById("msphoneAdmin").parentNode.id = "";
								this.user_molphoneFlag = true;
								if(this.user_adminFlag && this.user_telphoneFlag && this.user_molphoneFlag && this.user_emailFlag) {
									document.getElementById("user_ok").disabled = "";
								}
							} else {
								document.getElementById("msphoneAdmin").parentNode.id = "phoneInfo";
								this.user_molphoneFlag = false;
								document.getElementById("user_ok").disabled = "disabled";
							}
						} else {
							document.getElementById("msphoneAdmin").parentNode.id = "";
			...
			removeEquipGroup: function(dt) {
				...
				for(var i = 0; i < AlmReportDataLen; i++) {
					if(dtThis.AlmReportData[i].group_no == dt.group_no) {
						this.$Modal.warning({
							title: "提示",
							content: '<p style="position: relative;top: -4px;">请先删除管理范围菜单对应项!</p>'
						});
						return false;
					}
				}
				...
				function _success_admin_del() {
					WeekAlmReport.forEach(function(ele1, index1) {
						if(ele1.group_no == dt.group_no) {
							if(index1 != 0) {
								dtThis.selectEquip = WeekAlmReport[index1 - 1].group_no;
							} else if(WeekAlmReport.length > 1) {
								dtThis.selectEquip = WeekAlmReport[index1 + 1].group_no;
							} else {
								dtThis.selectEquip = -1;
							}
							WeekAlmReport.splice(index1, 1);
						}
					});
					console.log(dtThis.selectEquip)
					msg.success("操作成功");
				}
			},
			addEquipGroup: function() {
				...
				var AdministratorLocal = {
					equipcomb: "",
					group_name: "新增设备分组" + group_no_value,
			...
			saveEquipGroupName: function(dt, dtIndex) {
				var msg = this.$Message;
				msg.destroy();
				if(dt.group_name) {
					if(dt.group_name.length > 20) {
						msg.warning("设备分组名称不能超过20个字符");
						return false;
					}
					let equipUser = this.equipUser;
					let isExistFlag = false;
					for(var i = 0; i < equipUser.length; i++) {
						if(i != dtIndex && dt.group_name == equipUser[i].group_name) {
							isExistFlag = true;
							break;
						}
					}
					if(isExistFlag) {
						msg.warning("该设备分组名称已存在");
						return;
					}
			...
			allCheckbox: function(value) {
				...
				for(var i = 0; i < equipUser.length; i++) {
					if(equipUser.group_no == selectEquip) {
						isCheckFlag = true;
						break;
					}
				}
				//取列表名
				if(equipUser.length == 0 || isCheckFlag) {
					msg.error("请选择设备分组");
					return false;
				}
				if(value == "all")
			...
			saveUpdateAlmReportt: function() {
				...
				for(var i = 0; i < WeekAlmReport.length; i++) {
					if(!this.AlmReport_isjudege) {
						if(WeekAlmReport[i].id == weekID) {
							continue;
						}
					}
					if(WeekAlmReport[i].Administrator == WeekAlmReportLocal.Administrator &&
						WeekAlmReport[i].group_name == WeekAlmReportLocal.group_name) {
						isGroupFlag = true;
						break;
					}
				}
				if(isGroupFlag) {
					msg.warning("同一人员不能添加重复的管理范围");
					return;
				}
			...
			saveUpdateWeekAlmReport: function() {
				...
				//  判断时间
				if(!this.Week_admin) {
					this.$Message.warning('人员姓名不能为空');
					return;
				} else if(!this.Week_week) {
					this.$Message.warning('星期不能为空');
					return;
				...
				this.isTime();
				if(!this.isTime()) {
					this.$Message.warning('请输入正确时间格式');
					return;
				...
				for(var i = 0; i < WeekAlmReport.length; i++) {
					...
					if(WeekAlmReport[i].Administrator == WeekAlmReportLocal.Administrator && WeekAlmReport[i].week_day == WeekAlmReportLocal.week_day &&
						WeekAlmReport[i].begin_time == WeekAlmReportLocal.begin_time && WeekAlmReport[i].end_time == WeekAlmReportLocal.end_time) {
						isWeekFlag = true;
						break;
					}
				}
				if(isWeekFlag) {
					msg.warning("同一人员不能添加重复的周排表信息");
					return false;
				}
			...
			saveUpdateSpeAlmReport: function() {
				...
				if(isSpecFlag) {
					msg.warning("同一人员不能添加重复的特定日期排表信息");
					return false;
				}
			...
			XHRPost: function(api, updateJson, _success) {
				...
				this.Axios.post(urlna, updateJson, {
					...
					.catch(error => {
						...
						this.$Message.error("操作失败,"+error.response.data.HttpData.HttpMessage);
					});
			},
			//发送请求
			...
			XHRGet: function(url, _success) {
				...
				this.Axios({
					...
					.catch(error => {
						console.log(error.response.data);
						this.$Message.error("请求出错，请查看网络是否已连接！");
					});
			},
			deleteBaseData: function(api, json, _success) {
				let content = "";
				if(json.getDataTable == "Administrator") {
					content = "该人员";
				} else if(json.getDataTable == "EquipGroup") {
					content = "该分组";
				} else if(json.getDataTable == "AlmReport") {
					content = "该管理范围";
				} else if(json.getDataTable == "WeekAlmReport") {
					content = "该周排表";
				} else if(json.getDataTable == "SpeAlmReport") {
					content = "该特定日期排表";
				}
				this.$Modal.confirm({
					title: "提示",
					content: '<p style="position: relative;top: -4px;">确认删除' + content + '信息?</p>',
					okText: "确认",
					onOk: () => {
			...
			conditions: function(items) {
				var searchRegex = new RegExp(this.filtersValue, 'i');
				var arr = [],
					i = 0;
			...
			isTime: function() {
				...
				if(regexpSy.test(this.Week_etime) == false) {
			...
			isDate: function() {
				if(new Date(this.Spe_begin_time).getTime() >= new Date(this.Spe_end_time).getTime()) {
					this.Spe_isDate = true;
					return false;
				} else {
					this.Spe_isDate = false;
					return true;
				}
			},
