<template>
	<Row class="login" v-cloak>
		...
		<img src="../assets/img/logos-qh.png" alt="">
		</Col>
	...
	export default {
		...
		methods: {
			login() {
				...
				if(!this.userName && !this.userPwd && !this.verificationCode) {
					this.$Message.error("用户名和密码及验证码不能为空");
					this.loading = false;
					return false;
				} else if(!this.userName && !this.userPwd && this.verificationCode) {
				...
				let reqData = 'username=' + this.userName + '&userpwd=' + md5(this.userPwd) + '&verificationCode=' + this.verificationCode.toUpperCase().replace(/(^\s*)|(\s*$)/g, "");
